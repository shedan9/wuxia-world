...
2026-09-26 立面纹理与剩余件：
- 横向无缝的立面纹理（崖壁、驳岸）用 `size` 给非方形画幅（1536×640 / 1536×512）；错缝格线引导按宽度换算，`rows` 取“纹理宽 / 行高”，画幅外的格线自然裁掉。
- 石阶、木桥这类平整几何件：文生图要么把踏步画成一整块深色斜坡，要么在框里另画一座小桥；图生图只照描平涂占位。简单平面宜用纹理贴面，不走逐件出件。
- 井台改写提示词（八角石井圈、木架、滑轮吊桶）并用 control 0.6 后，六个种子里 55、66 画全了井身。
- 廊棚屋面首轮瓦垄像刷痕，提示词写“clearly visible parallel rows of curved grey clay tiles”并把 control 提到 0.9 后成行。

2026-09-26 溪涧（`jobs/m0_creek.json`，每张约 7 秒）：
- 溪岸卵石立面：`texture_wall` 风格下 11 号偏卡通贴纸、44 号大片空底，取 66（石块大、灰绿相间、明暗自然）；1536×512 贴在 40 高的岸上会把卵石压得很碎，按纹理宽高比只取上部、横向 200 单位一格。
- 用户看过后指出岸与水底“全是规整鹅卵石”：岸改为“长苔湿土与基岩、一两块大棱石”（`negative_bank2` 排除 cobblestone / pebble wall / rows of stones），取 k5·44；k4 一组“土岸 + 树根”画成树根特写或贴纸圆洞，不用。水底改为细沙（`negative_bed` 排除 pebbles），取纹理最平淡的 k6·11，散石由着色器稀疏叠加（与土路碎石同理）。
- 水面：提示词写“清浅溪水”时模型几乎只画水底卵石，“stylized water surface”一组则变成土黄石块或浅滩地图。改为只取溪床卵石纹理（k2·44），水色、浅深两阶、浪线与近岸白沫都由着色器叠加。

2026-09-26 木纹（`jobs/m0_wood.json`，每张约 7 秒，2 组 × 6 种子）：
- 朱漆木六张都偏粉红、木纹对比很强，取最匀净的 w1·55；整贴在细柱上像蛇皮，改为程序化朱漆底色上以 35% 不透明度叠纹理。
- 风化木取横纹匀净的 w2·11（33 号色带太重，其余多为竖纹）；纹理只在四边形面上贴，圆木端头保留底色。
