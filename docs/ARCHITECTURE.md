...
**美术来源（2026-09-24 用户确定）：美术全部采用 AI 生成或免费资源。** 人物、场景、道具与图标由本地 SDXL 生成底稿（`tools/ArtGen`，环境与复现步骤见其 README），人工挑选修整后入 `art_source/`；写意远景与纸墨肌理可取博物馆 CC0 古画裁切调色；字体用 OFL 开源字体。简单资源（图标、肌理、UI 装饰、构图小样）在开发笔记本（RTX 4060 8 GB）生成，复杂资源（立绘、三视图、场景大图与高清放大）在家用台式机（RTX 4070 Ti SUPER 16 GB）生成。每项资产的来源、许可与生成记录登记在[资产台账](./art/ASSET_LEDGER.md)；模型或 LoRA 须允许商用。人物跨立绘、探索和战斗形象的一致性是 AI 管线的主要风险，M0 以一位人物的完整样板先验证。
...
| 间距 | 8 / 16 / 24 / 32 / 48 |
| 宣纸选中底（M0 补充） | `#E4D6B8`，列表选中与悬停底色 |
| 宣纸次级文字（M0 补充） | `#5E6159`，对宣纸约 5.3:1 |
| 深底次级文字（M0 补充） | `#B8AE98`，对深色面板约 6.8:1 |
| 青山（M0 补充） | `#3F5A4E`，数值提升与内力条，须配 ▲ 等符号 |
...
**M0 实现（2026-09-24）。** 语义色与尺寸集中在 `game/scripts/presentation/UiPalette.cs`；`Ui/UiTheme.cs` 以代码生成全局 `Theme`，由 `AppHost` 挂到根窗口。基础控件按宣纸面配色，深色面与特殊用途使用类型变体（`PrimaryButton`、`RowButton`、`ChipButton`、`SpineTab`、`DarkButton`、`SheetPanel`、`InsetPanel`、`SealPanel` 等），页面只选择变体、不各自覆写颜色。`Ui/Ui.cs` 提供标签、列表行、开关、印章、字形图块等工厂方法。

- 界面版式为“册页”：深色底上左上角一方朱砂印章写竖排页名，左侧竖排页签，选中页签与中间宣纸页面连成一片；列表选中以加深底色和左侧朱砂竖线表示，开关文字同时写“开/关”。
- 字体随包：`assets/fonts/` 下思源黑体 CN（正文）与霞鹜文楷 Medium（标题、印章、页签），`UiFonts` 加载；导出脚本把 OFL 许可与资产台账复制到 `build/windows/licenses/`。
- 物品、招式与见闻来源在正式图标完成前使用“字形图块”（单字加类别色描边）示意，不作为图标美术验收；角色页立绘位置标“立绘待制作”。
- 截图命令：`Godot --path game --resolution 1920x1080 -- --scene=<场景> --tab=<页签序号> --capture=<png>`，由 `Presentation/App/DevCapture.cs` 处理，未传参数时不生效，用于交付截图与布局自查。
