...
**2026-09-24 用户改定：弃用水墨宣纸风，改为蓝绿清新基调、偏写实的卡通人物。** 人物立绘采用写实比例的赛璐璐/轻厚涂：清晰线稿、两到三阶明暗、五官与肌肉结构写实但保留卡通化的眼神和发丝处理（用户参考图 `docs/art/example/` 为同类国风游戏立绘，只作风格参照，不作为图生图输入或训练素材）。整体色调以青绿、湖蓝、竹青与白为主，江南水色与晴光取代宣纸、墨色和旧金；暖色（朱红、杏黄）只做人物服饰点缀；界面上唯一的暖色是杏红，只用于气血、下降与警示。人物正常比例约 6.5–7.5 头身，成年角色面部不做低龄化，衣物、兵器和姿态承担门派辨识；不使用像素化滤镜、低分辨率放大或最近邻采样制造风格。
...
**地图与城镇的生产方式（2026-09-24 拟定，待用户确认）。** AI 无法一次画出视角一致、可行走、可拼接的整张地图，因此按“人定布局、AI 出件、引擎拼装”分三层：
...
2026-09-24 用户改定为蓝绿清新方向，界面配色取“青绿山水”：石青、石绿为主，水白为浅色页面、深潭为深色面，杏红是唯一暖色，只用于气血、下降和警示。成员名改为语义名（旧名 `Paper`→`Surface`、`Ink`→`Text`、`Cinnabar`→`Accent` / `Warm`、`OldGold`→`Trim`、`Mountain`→`Boost`）。对比度按 WCAG 相对亮度实测计算。

| 语义（`UiPalette`） | 颜色 / 尺寸 | 用途与对比度 |
|---|---|---|
| 水白 `Surface` | `#F2F8F6` | 浅色页面底 |
| 选中底 `SurfaceShade` | `#DCEEEA` | 列表选中与悬停 |
| 正文 `Text` | `#173A3F` | 对水白 11.4:1 |
| 次级文字 `TextMuted` | `#4A686B` | 对水白 5.6:1，对选中底 5.0:1 |
| 深潭 `PanelDark` | `#15424A` | 深色面板与外框 |
| 深底正文 `TextOnDark` | `#EDF8F5` | 对深潭 10.1:1 |
| 深底次级 `TextOnDarkMuted` | `#A3CBC6` | 对深潭 6.2:1 |
| 石青 `Accent` | `#1D6A8A` | 主要按钮、选中竖线、页名章、滑杆；水白字在其上 5.6:1 |
| 石绿 `Trim` | `#6FB3A2` | 边框与装饰线，不作正文（对水白仅 2.3:1） |
| 竹青 `Boost` | `#26704F` | 提升、增益；对水白 5.6:1，须配 ▲ 等符号（内力条 2026-09-24 改用石青渐变，与增益分开） |
| 杏红 `Warm` | `#B04A36` | 气血条、下降、敌方意图、药品类；对水白 5.0:1，对选中底 4.5:1 |
| 金泥 `Gilt` | `#D2BC82` | 2026-09-24 增补：角饰、菱形标记、焦点折角、稀有度与深色面短标签；对深潭 5.9:1、对玄潭 8.1:1；不作大面积底色与玉版正文 |
| 玄潭 `Abyss` | `#0B2A30` | 2026-09-24 增补：深色渐变底端、遮罩、投影与键帽文字；深底正文对其 13.9:1 |
...
**M0 实现（2026-09-24，同日重做为游戏化界面）。** 第一版“册页”界面（纯色深潭底、竖排页签、白色页面）被用户认为像网页、过于粗糙，已按[界面设计规范](./art/UI_DESIGN.md)（“青绿·玉版”）重做；视觉语言、组件状态、各界面版式、动效与输入以该文件为准，本节只记录结构与接口。语义色与尺寸集中在 `game/scripts/presentation/UiPalette.cs`；`Ui/UiTheme.cs` 以代码生成全局 `Theme`，由 `AppHost` 挂到根窗口，页面只选择类型变体、不各自覆写颜色。

- 纹饰样式框 `Ui/OrnateBox.cs`（继承 `StyleBox`，RenderingServer 绘制）：切角、两色渐变、外框与内衬线、回纹 / 折角角饰、左侧标记条、菱形、高光线与柔和投影，不依赖贴图。全部面板、按钮、进度条与滑杆状态由它组合。
...
- 组件工厂 `Ui/Ui.cs`：列表行、开关、页名章、字形印鉴、菱形分隔 `DiamondRule`、小节标题、键帽提示、锚点摆放 `Place`、整树忽略鼠标 `IgnoreMouse`。动效 `Ui/Motion.cs`（入场、依次入场、呼吸、淡入淡出；`Motion.Enabled` 为 false 时直接落到终态，截图模式自动关闭）。
- 背景 `Presentation/Scenery/Backdrop.cs` + `assets/shaders/landscape.gdshader`：程序化青绿山水（天空、日轮、流云、四重山、流雾、江面、近岸）与柳叶粒子，参数 `defocus`、`veil`、`left_wash`、`parallax`、`world_seed`；标题页清晰、菜单虚化压暗、存档缩略图静止。正式场景美术完成前它也作探索与战斗的占位远景。
...
- 截图命令：`Godot --path game --resolution 1920x1080 -- --scene=<场景> --tab=<页签或状态序号> --capture=<png>`，由 `Presentation/App/DevCapture.cs` 处理，未传参数时不生效，用于交付截图与布局自查；标题、对话、战斗、探索 HUD 页的 `--tab` 表示界面状态，含义写在各页面类注释中。
