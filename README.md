...
当前处于 M0 起步阶段：已按架构文档搭建工程骨架（Godot 工程、场景目录框架与规则类库空项目），已在本机完成 `dotnet build`、Godot 导入与 Windows x64 导出。界面已按[界面设计规范](./docs/art/UI_DESIGN.md)重做为游戏化的“绢本青绿”风格（青绿山水背景，绢色与黛色面板、朱砂印章、笔触框线）：12 个展示页中已完成标题页、人物对话、回合战斗、探索 HUD（界面层）与角色、背包、札记、设置共 8 页（固定样例数据，战斗与探索形象为剪影、场景为程序化山水占位）；大地图与三类探索布景依赖尚未制作的美术样板，因此还不是可交付的视觉 Demo。目标平台仅 Windows x64。
